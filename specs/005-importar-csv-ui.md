...
5. Al confirmar, recién ahí se ejecuta la importación real (misma lógica del spec 001: normalización de encoding, parseo, upsert por OT, baja lógica de OT ausentes).
   - **Corrección de rendimiento** (encontrada con el archivo real, miles de filas): la primera versión resolvía región/comuna y hacía upsert **fila por fila, en secuencia** — con miles de filas eso son decenas de miles de round-trips secuenciales a la base, suficiente para que la función de Vercel corte la ejecución con 504 a la mitad de una importación (dejando algunos Casos procesados y otros no, y sin llegar a correr la baja lógica del final). Ahora `syncCasos` resuelve región/comuna **una sola vez por valor único** (no por fila) y hace los upserts de Casos **en paralelo, por lotes de 20**, en vez de uno a la vez. También se subió `maxDuration` de la función a 60s como margen adicional.
6. Mientras se calcula la vista previa o se confirma la importación, se muestra una barra de progreso indeterminada (no hay manera de conocer un porcentaje real de avance, así que no se finge uno) y, durante la confirmación, un aviso de que archivos grandes pueden tardar varios minutos.
